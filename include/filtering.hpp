...
double filterPixel(std::vector<std::vector<int>> image, std::vector<double> _weights, int n, int patchSize, int pixelRow, int pixelCol, double sigma)
{
    double res = 0;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            dist = util::computeEuclideanDistance(image, _weights, n, patchSize, pixelRow, pixelCol, i, j);
